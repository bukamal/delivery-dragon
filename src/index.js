...
app.get('/api/shop/orders', async (req, res) => {
  try {
    ...
    const orders = await pool.query(`SELECT o.*, u.name as customer_name FROM orders o JOIN users u ON o.customer_id=u.id WHERE o.shop_id=$1 AND o.status IN ('paid','preparing','ready_for_pickup','delivering','completed') ORDER BY o.created_at DESC`,[shop.rows[0].id]);
